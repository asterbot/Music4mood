...
def main():
    ...
    max_tempo=min(emotions_value['Fear']+0.5,1)
    min_tempo=max(emotions_value['Fear']-0.5,0)
    max_valence=min(emotions_value['Happy']+0.5,1)
    min_valence=max(emotions_value['Happy']-0.5,0)
    max_energy=min(max(emotions_value['Happy'],1-emotions_value['Sad'])+0.5,1)
    min_energy=max(min(emotions_value['Happy'],1-emotions_value['Sad'])-0.5,0)
    max_danceability=min(1, emotions_value['Happy']+0.5)
    min_danceability=max(0,emotions_value['Happy']-0.5)
    max_liveness=min(1, emotions_value['Happy']+0.5)
    min_liveness=max(0,emotions_value['Happy']-0.5)
    max_loudness=min(1, (emotions_value['Surprise']+emotions_value['Angry'])/2+0.5)
    min_loudness=max(0,(emotions_value['Surprise']+emotions_value['Angry'])/2-0.5)
